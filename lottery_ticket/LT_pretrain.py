...
class LineByLineTextDataset(Dataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        # write input_ids in a text file
        with open("input_ids.txt", "w") as f:
            for i in range(len(self.examples)):
                f.write(str(len(self.examples[i])) + "\n")
...
def pruning_model(model, px):
    ...
    for ii in range(2):
        parameters_to_prune.append(
            (model.bert.encoder.layer[ii].attention.self.query, 'weight'))
        parameters_to_prune.append(
            (model.bert.encoder.layer[ii].attention.self.key, 'weight'))
        parameters_to_prune.append(
            (model.bert.encoder.layer[ii].attention.self.value, 'weight'))
        parameters_to_prune.append(
            (model.bert.encoder.layer[ii].attention.output.dense, 'weight'))
        parameters_to_prune.append(
            (model.bert.encoder.layer[ii].intermediate.dense, 'weight'))
        parameters_to_prune.append(
            (model.bert.encoder.layer[ii].output.dense, 'weight'))
...
def see_weight_rate(model):
    ...
    for ii in range(2):
        sum_list = sum_list + \
            float(
                model.bert.encoder.layer[ii].attention.self.query.weight.nelement())
        zero_sum = zero_sum + \
            float(
                torch.sum(model.bert.encoder.layer[ii].attention.self.query.weight == 0))

        sum_list = sum_list + \
            float(
                model.bert.encoder.layer[ii].attention.self.key.weight.nelement())
        zero_sum = zero_sum + \
            float(
                torch.sum(model.bert.encoder.layer[ii].attention.self.key.weight == 0))

        sum_list = sum_list + \
            float(
                model.bert.encoder.layer[ii].attention.self.value.weight.nelement())
        zero_sum = zero_sum + \
            float(
                torch.sum(model.bert.encoder.layer[ii].attention.self.value.weight == 0))

        sum_list = sum_list + \
            float(
                model.bert.encoder.layer[ii].attention.output.dense.weight.nelement())
        zero_sum = zero_sum + \
            float(
                torch.sum(model.bert.encoder.layer[ii].attention.output.dense.weight == 0))

        sum_list = sum_list + \
            float(
                model.bert.encoder.layer[ii].intermediate.dense.weight.nelement())
        zero_sum = zero_sum + \
            float(
                torch.sum(model.bert.encoder.layer[ii].intermediate.dense.weight == 0))

        sum_list = sum_list + \
            float(model.bert.encoder.layer[ii].output.dense.weight.nelement())
        zero_sum = zero_sum + \
            float(
                torch.sum(model.bert.encoder.layer[ii].output.dense.weight == 0))
...
def rewind(pre_weight):
    ...
    name_list = []
    for ii in range(2):
        name_list.append('bert.encoder.layer.'+str(ii) +
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            outputs = model(inputs, masked_lm_labels=labels) if args.mlm else model(
                inputs, labels=labels)  # this line again
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # print(prediction_scores.size(-1))
            vocab_size = prediction_scores.size(-1)
            acc = accuracy(prediction_scores.view(-1,
                           vocab_size).data, labels.view(-1))[0]
...
def csv_to_text(csv_file_loc):
    import pandas as pd

    file_name = csv_file_loc.split("/")[-1].split(".")[0]
    txt_name = csv_file_loc.replace(".csv", ".txt")
...
def main():
    ...
    parser.add_argument(
        "--model_type", default='bert', type=str, required=True, help="The model architecture to be trained or fine-tuned.",
    )
    ...
    parser.add_argument("--noise_rate", default=0.01,
                        type=float, help="The noise rate for weight.")
    ...
    parser.add_argument("--logging_steps", type=int,
                        default=10000, help="Log every X updates steps.")
